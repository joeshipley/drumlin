...
|---|---|
| *Move* `dsp_core` out of `synth_core` (M1) | **Copy** the pure-DSP modules verbatim into a new `dsp_core` crate *inside* Drumlin. Keep them byte-for-byte identical so they could be upstreamed/merged into Esker later. |
| Esker's golden test proves the move was zero-change | We add our **own** golden coverage; we also keep dsp_core source diff-clean against Esker's so a future merge is trivial. |
...
This costs us the *literal* shared crate (a DSP fix must be applied in both repos), which is acceptable and was the chosen trade-off. Everything else in the design doc stands.
...
- **M4** — Full 12-voice kit: toms (LO/HI), perc/rim/cowbell, ride/crash, FM-perc, noise/FX, sample/user. Per-voice golden one-shots for each.
- **M5** — Full step grid: per-step velocity, micro-timing, probability, ratchet/roll + ramp, conditional trigs, **per-step p-locks** (`LOCKABLE_PARAMS` + reconcile test), per-lane length (polymeter), Euclid generator, swing + groove templates, 16 patterns + song chaining, FILL button, live record/quantize. Golden-trigger fixture + determinism + polymeter-convergence tests.
