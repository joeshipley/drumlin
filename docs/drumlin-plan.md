...
- **M8** — Mixing + the VOICE/MIX editors. In progress:
  ...
  - **Remaining (next chunks):** the **MIX** page (channel strips: fader/mute/solo, per-voice **Send A/B**), the snare **gated-verb** (per-voice reverb routing + a new return-gate primitive — pairs with Send A), **multi-out** routing (aux output ports in the audio-IO layout + AU/CLAP), and optional host sidechain aux input. Per-voice pitch/decay trims and type-specific engine knobs are not lockable yet and arrive with the M6 mod infrastructure.
