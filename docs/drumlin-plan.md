...
- **M8** — Mixing + the VOICE/MIX editors. In progress:
  ...
  - **Remaining M8 (still open):** a **2-band trim EQ** per strip, a MIX-page **choke-group selector** (the engine `choke_group` exists; just unsurfaced), **multi-out** output routing (declare aux output ports — touches the audio-IO layout + the AU/CLAP build, so likely its own follow-on phase), the snare **gated-verb** (per-voice reverb tap + a new return-gate primitive — pairs with Send A), and optional host **sidechain aux** input. Per-voice pitch/decay trims + type-specific engine knobs are not lockable yet and arrive with the M6 mod infrastructure.
