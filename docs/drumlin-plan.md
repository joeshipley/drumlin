...
- **M8** — Mixing + the VOICE/MIX editors: per-voice channel strips, **Send A/B** (the snare **gated-verb** send lands here — per-voice reverb routing + a new return-gate primitive), **multi-out** routing (aux output ports in the audio-IO layout + AU/CLAP) and optional host sidechain aux input. The five per-voice tail params (Level/Pan/Cutoff/Resonance/Drive) are already plumbed end-to-end via the p-lock system, so the editor largely exposes existing wiring (plus a `DrumKit` patch-default setter + an editor→kit edit channel). Per-voice patches join the persisted `PersistState` once the VOICE editor makes them user-editable.
