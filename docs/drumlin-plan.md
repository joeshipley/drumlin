...
- **M5** — Full step grid. Split into two passes:
  ...
  - **M5 part 2 (next):** pattern bank (16 patterns) + song chaining, Euclid generator, FILL button + live record/quantize, groove-template library, per-track speed, and the **rich step-editing GUI** (p-lock popover, velocity ribbon, probability pips, ratchet/condition editor). Plus the part-1 review's deferred refinements: negative-micro/flam via one-step look-back scheduling, sub-block chunking to remove the carry/pending caps, and a filter "snap" path so per-step Cutoff locks step rather than glide (the tail's smoother currently slews them over ~5 ms).
