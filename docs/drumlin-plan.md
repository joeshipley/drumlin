...
- **M7** — Full bus FX. In progress:
  ...
  - **Part 3 (next):** transient shaper (new shared-core DSP), parallel/NY comp, the snare gated-verb send, and a pump rate/curve control.
...
- **M8** — Mixing: per-voice channel strips, sends, **multi-out** routing (aux output ports in the audio-IO layout + AU/CLAP) and optional host sidechain aux input. Per-voice patches join the persisted `PersistState` once the VOICE editor makes them user-editable.
