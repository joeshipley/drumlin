...
- **M11 (in progress) — THE DIG: seed-addressable groove excavation.** The stand-out feature: a generative pattern engine, themed to the name (a drumlin is what a glacier leaves behind — you *excavate* grooves from a seeded landscape). Five pillars:
  ...
  **Architecture:** entirely editor-side in the plugin crate (`crates/drumlin/src/dig.rs`) — a pure `fn(terrain, knobs, seed) -> Pattern`, the `worlds.rs` builder shape. Audition = the existing queued-pattern-switch (in-tempo, loop-boundary, cancellable); KEEP = the existing undo-snapshotted recall path. **No `synth_core` change, no `percussion_core` change → the 13 goldens are untouched by construction.** Chunks: **c1 ✅ generator core** (role grammars, knob transforms, scoring, `dig_one`/`dig_best`, TECHNO + BREAKS terrains, property tests: determinism, density monotonicity, payload validity, anchor guarantee, end-to-end finite render); **c2 ✅ the full terrain set + THIS-WORLD** — DISCO (four-on-the-floor + the committed off-8th open hat), HALFTIME (backbeat ONLY on beat 3; 4/12 silent by authoring), FOOTWORK (the 3-3-2 kick lattice, melodic toms), plus the world dialects CAVERN (Bladerunner: sparse, toms carry the groove, ride-bell motor) and OUTRUN (gated backbeat on 4+12, late-bar tom fill); `terrain_for_world()` maps every factory kit (Neutral/unknown → techno). The score became terrain-aware where it must be: the density term compares against the terrain's own *expected* hit count at the current knobs (a sparse cavern isn't punished for being sparse) and the motor term follows whichever lane the terrain actually commits as its motor (the cavern's is the ride). Dialect-character tests run at WILD=0, where a 0-prior cell is a guaranteed rest — signatures are structural, not luck; **c3 ✅ the DIG page** — a DIG rail view (terrain chips incl. THIS WORLD via the kit_id tag, four knobs, six candidate cards with mini-grids: ghosts dimmed, accents lit, seed hex + score dots). **Audition is in-tempo**: a new pattern *mailbox* (`dig_stage: Arc<Mutex<Option<(slot, Pattern)>>>`, drained by `try_lock` at block start BEFORE the edit ring) hands the candidate to `Sequencer::load_slot` (new, content-only — live pattern updated when loading the current slot so the live→bank flush can't clobber) into scratch slot 16, then the normal queued switch lands it on the loop boundary — no recall, no playhead reset, the groove never stops; ■ RETURN switches back the same way. **KEEP** commits to the first free slot: undo-snapshotted (the shared slot-tagged `push_undo`), persisted-state mirrored under the lock (a host save mid-keep is coherent), staged + selected. The GUI writes its bank mirror + feel meta from the full cells payload (a Rust test pins that the cells mirror the pattern exactly, so later grid edits on a kept dig send truthful values); **c4 ✅ mutate/locks/breed** — generation as collaborator: a MUTATE toggle digs VARIATIONS of the current pattern (engine-truth source: the persisted current slot). Per-track LOCK chips carry lanes through bit-identically (the whole `Track` — notes, p-locks, polymeter length; and the source's feel/seed is preserved so locked lanes *play* identically too); DEPTH is the per-cell probability that an unlocked cell is re-excavated from the grammar — per-cell S&H draws make it accumulate monotonically (deeper = more of the SAME variation, never a reroll; depth 0 is bit-exact identity, pinned by test). `splice(a,b,seed)` is lane-level crossover (each track wholly from one parent — A's kick with B's hats, never cell-salad), surfaced as **⚡ BREED** on the auditioned card: offspring = that candidate × fresh terrain rolls, locks forcing its lanes, scored and re-surfaced. **c6 ✅ MIDI export** (pulled ahead of c5 by request — the TR-909-software flow): `midi_export.rs` writes a Standard MIDI File (format 0, PPQ 480, GM drum channel 10) of ONE baked pass exactly as the engine plays loop 0 — swing/groove/micro/seeded-humanize as tick offsets (the GROOVE LOCK makes "as heard" reproducible), conditions gated at loop 0, the frozen probability rolls, ratchets expanded with their velocity ramps, accents folded into velocity, polymeter lanes wrapped across the bar, muted lanes skipped. Notes use `TRACK_NOTE` — the pinned inverse of `track_for_note` (test-enforced), so a dragged region drives Drumlin 1:1 (and any GM drum instrument). No tempo meta: ticks are musical time, the region conforms to the project tempo. Files land in `~/Music/Drumlin/` and are revealed in Finder for the drag (true in-window drag-out needs `NSFilePromiseProvider` plumbing in the vendored webview — deferred). GUI: a **MID ⤓** header chip exports the current pattern (`Drumlin-P<nn>.mid`), every dig card exports directly (`Drumlin-DIG-<SEED>.mid`, no keep needed), confirmation flash. What can't ride a static region: p-locks, choke, drift (the notes are portable; the SOUND stays in the instrument). Two additive percussion_core pub-ifications (`TrigCondition::passes`, `GrooveTemplate::offset_frac`) so the export bakes with the engine's own math. 5 writer tests incl. a structural SMF parser. **c5 ✅ the full MOTION layer — M11 COMPLETE.** Two halves. *Engine truth-fix:* probability now **breathes per loop** — the roll mixes the loop index (`PURPOSE_PROB_LOOP = 7`) into the per-cell seed, so a 50% step actually flickers pass to pass, while the whole performance stays a pure function of `(seed, loop)`: replaying the same transport span reproduces bit-exactly (test-pinned both ways; the old frozen-per-loop pin test was rewritten to the new contract; FEEL draws — humanize, drift — stay per-cell frozen: the hand is consistent, the dice are alive). Goldens unaffected (no probability steps at Neutral); the MIDI export bakes the same loop-0 roll. *Dig writes* (`apply_motion`, role-aware, per-cell seeded APPLY/VALUE draw pairs → monotonic in the knob): **pre-wired FILLS** (extra late-bar Color/Backbeat hits with `condition: Fill` — silent until the FILL button, so every dig arrives with its fill built; shown as hollow cells in the candidate minis; excluded from the score's sounding-pass stats and from MIDI regions), NotFill duck-outs on non-identity motor hits (space for the fill to speak), Ratio 1:2/2:2 every-other-bar evolution on Color, ratchet ramps (builds on motor rolls, dying flam drags on backbeat ghosts, building fill rolls), Cutoff/Pitch/Decay p-locks (≤2 per step, the hat filter dance + melodic stabs), and micro feel (backbeats lay back, color pushes/pulls, motors stay tight, anchors stay pure). Tests: motion-0 = fully static, motion-1 writes every payload kind (deterministic seed scan), and the headline integration — a dig's Fill step sounds ONLY while FILL is held. Real-time MIDI-out (record Drumlin onto a host track, Drumlin-sequences-Esker) stays the §Later item.

- **M12 (in progress) — THE KIT LIBRARY.** Goal: a 50+ factory library, built without decision fatigue: **batches of five kits, each ending in a listening session** (the M9 lesson — technically-fine kits still need ears; drafts come from Claude, taste comes from Joe). Strategy: an **archetype matrix** (~12 families — 808 heritage, 909/house, techno warehouse, electro, boom-bap, trap/halftime, DnB/breaks, garage, dub, industrial, 80s gated, ambient — × ~4 variants each), authored through the capture loop rather than hand-written Rust. Structural decision: **kits don't need patterns** — each `Kit` carries a `terrain` dialect tag and THIS-WORLD digs supply infinite grooves in-genre (`terrain_for_world` now reads the tag; user kits carry theirs in JSON).
  **c1 ✅ the capture loop:** `.kit.json` (`drumlin-kit-v1`; serde on `KitRow`, kebab-tagged; `KitDef` = name/blurb/terrain/labels/rows) in `~/Music/Drumlin/Kits/` (size-capped, sanitized, dir-param tested). **EXPORT KIT** captures the machine's sound as its *minimal lens* — `diff_kit` emits only rows differing from Neutral, using the exact inverse encodings the recall decodes (`LockableParam::normalize`, the new `VoiceMixRow::get` — `set∘get` identity), tags the current DIG dialect, writes + reveals in Finder, honesty pinned by `exported_kit_reproduces_the_machine` (stage∘diff = identity on the lens surface). **MY KITS** on the KITS page lists the folder live (each full re-seed refreshes it — drop a JSON in, it appears), recalled atomically via `stage_kit_def` (the factory `stage_kit` refactored into shared `stage_reset`/`stage_rows`); a recalled user kit's dialect drives THIS-WORLD digs (`resolve_dig` handles `user:` ids by reading the file). Authoring loop: **design in Logic with ears → EXPORT KIT → tweak JSON → recall → listen** — seconds, not rebuild-minutes.
  **c2 ✅ the bake step:** `cargo xtask bake-kits` transcribes `crates/drumlin/kits/*.kit.json` (the version-controlled graduated library; README documents the flow) into generated `src/kits_baked.rs` — one `&'static Kit` per file + `BAKED_KITS`, chained into the new `kits::factory_kits()` iterator (core worlds + baked; all call sites migrated). The tool eagerly validates structure/ranges with per-file error messages (format tag, 8 labels, per-variant field ranges incl. the raw-count mix fields, non-Off mod slots, id collisions vs core) but the real guards are crate-side: the generated file must compile against the actual `KitRow`, and the factory tests sweep every baked kit automatically — row decode, **id uniqueness + strict terrain registration (new test)**, staging, and the finite-render gate. Deterministic output (filename-sorted; kit id = kebab-cased stem). Proven end-to-end with a throwaway kit (all 7 row kinds JSON→Rust→compiled→all tests green), then re-baked to empty. **c3+** content batches of five (archetype matrix above, one family per session); later spice: VARY (seeded bounded kit mutation around a base, DIG-style locks), promote-preset-to-kit.
